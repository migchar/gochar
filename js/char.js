...
// 解决Bootstrap4 导航栏 fixed-top 后，锚点定位时遮挡问题
// 利用锚点改变事件onhashchange，使用jQuery的scrollTop向前滚回导航栏的高度
...
function searchEvent() {
    $('.btn--search').click((event) => {
        ...
        // 添加搜索界面隐藏样式
        $('.search-shade').addClass('search--open');
        $('.main-wrap').addClass('main-wrap--move')
        $('.search__input').focus();
    });

    $('.btn--search-close').click((event) => {
        // 恢复滚动
        $('body').removeClass('overflow-hidden');
        // 移除搜索界面隐藏样式
        $('.search-shade').removeClass('search--open');
